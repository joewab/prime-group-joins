...
SELECT SUM(products.unit_price * line_items.quantity) AS total_order_price, orders.id
FROM (orders JOIN line_items ON orders.id = line_items.order_id) JOIN products ON products.id = line_items.product_id
GROUP BY orders.id
